define("easyhin/login/1.0.0/oauth-debug", [ "jquery-debug", "easyhin/common/1.0.0/common-debug.js" ], function(require, exports, module) {
    ...
    var REG = {
        ...
        bindEvent: function() {
            var _this = this;
            $("#username1,#passwd1").keyup(function(e) {
                if (this.value != "") {
        ...
        showTips: function(o, ty) {
            $(o).nextAll("i").attr("class", "icon2 " + ty);
        },
        set: function() {
            if ($.trim($("#username1").val()) == "") {
                this.showTips($("#username1"), "alt");
                return false;
            }
            if ($("#passwd1").val() == "") {
                this.showTips($("#passwd1"), "alt");
                return false;
            }
            var m = md5(), pwd = $("#passwd1").val();
            if (pwd.length <= 16) {
                pwd = m.hex_md5(m.hex_md5(m.hex_md5(pwd))).toUpperCase();
            }
            $.ajax({
                url: "/index.php?c=register&m=oauth",
                data: {
