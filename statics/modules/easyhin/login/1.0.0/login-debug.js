define("easyhin/login/1.0.0/login-debug", [ "jquery-debug", "easyhin/common/1.0.0/common-debug.js" ], function(require, exports, module) {
    ...
    var LOGIN = {
        init: function() {
            if (Cookie.get("un")) {
                var obj = $("#username");
                obj.val(Cookie.get("un"));
                $(obj).next().addClass("hidden");
                this.showTips(obj, "ok", "");
            }
            this.bindEvent();
        },
        bindEvent: function() {
        ...
        set: function() {
            ...
            $.ajax({
                url: "/wei/index.php?c=login&m=run",
                data: {
                ...
                cache: "false",
                beforeSend: function() {
                    $("#submit").html("登录中").addClass("disabled");
                },
                error: function() {
                    alert("系统繁忙，请稍后重试");
        ...
        verifyFields: function() {
            ...
            if ($.trim($("#username").val()) == "") {
                this.showTips($("#username"), "alt", "请输入登录帐号");
                checked = false;
            }
            if ($("#passwd").val() == "") {
                this.showTips($("#passwd"), "alt", "请输入登录密码");
                checked = false;
            }
            return checked;
        }
