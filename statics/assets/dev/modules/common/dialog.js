...
define(function(require, exports, module) {
	...
	var $           = require('jquery/jquery')
		, _float    = require('common/float')
		, _template = require("common/tplparser")
		, _baseFun  = require('common/fun')
        , _floatmask = require("common/floatmask")
		, _config   = {
			submitId    : 'wd-dialog-submit' //模板里面的 id
			, closeId   : 'wd-dialog-close'
		}//缺失参数  参照 legos4 : float
		, _defaults = {
			autoResize  : true
			, title     : ''
			, style     : "none"
			, dragble   : false //  float 组件 用的！ 会有点问题，会导致 整个框架都能拖动！
			, dragbled  : true //  标题是否可以拖动，自己写的 拖动
			, showClose : false
			, onInited  : _baseFun.noop() //自定义传进来的 关闭事件。  float 用的是   onInit
			, onConfirm : _baseFun.noop() // onConfirm( opts ) ; 自定义传进来的 关闭事件。  float 用的是   onInit
			, onClosed  : _baseFun.noop() //自定义传进来的 关闭事件。  float 用的是  onClose
			, zindex    : 99999 //number, 把zindex作为参数传入
			, width     : '400'
			, closeId   : '' // 模板里面的 id 逗号分隔
			, delayClose: 0 // 延迟 关闭，单位：秒
		}
		, _tpl = {
			tips :
				'<div class="ui_popup">'+
				'<i class="icon_<%=@type%>"></i>'+
				'<div class="ui_popup_container"><%=@html%></div>'+
				'<div class="ui_popup_action">' +
				'   <% if( "confirm" == @type ){ %>' +
				'   <a id="'+ _config.submitId +'" class="btn" href="###">确定</a><a id="'+ _config.closeId +'" class="btn" href="###">取消</a>'+
				'   <% }else{ %>' +
				'   <a id="'+ _config.closeId +'" class="btn" href="###">关闭</a>'+
				'   <% } %>'+
				'</div>'+
				'</div>'
			...
			, dialog : '<div class="ui_dialog_wrapper gallery" data-tag="gallery" style="left:0; top:0;background: #FFF; position: relative;display:block;">'+
				'   <div class="ui_dialog">'+
				'   <div class="ui_dialog_header" data-tag="dialog_header">' +
				'       <div class="ui_dialog_title"><%=@title%></div>'+
				'       <a class="ui_dialog_close" href="###" title="关闭" id="'+ _config.closeId +'">×</a>'+
				'   </div>'+
				'   <div class="ui_dialog_container"><%=@html%></div>'+
				'   <div class="ui_dialog_footer">'+
				'       <% if( @isShowOk ){ %>' +
				'       <a href="###" class="btn" id="'+ _config.submitId +'">确定</a>'+
				'       <% } %>'+
				'   </div>'+
				'   </div>'+
				'</div>'
	...
	function openFloat( title, html, opt, type ){
		...
		if( type == 'dialog'){
			...
			opts.html = tplDialog.render({
				title       : title
				, html      : html
				, isShowOk  : opts.isShowOk //是否展示 确定
			});
		}else if( type == 'success'){
			opts.delayClose = 3e3;
			opts.cover = false; //不要背景遮罩
			//内容
		...
		if( opts.delayClose > 0 ){
			//floatOpen.close();
			$( '#'+ floatOpen.boxId).animate({ opacity : 0}, opts.delayClose, function(){
				floatOpen.close();
			});
			setTimeout(function(){
				ui( 0 );
	...
	//界面 事件
	function ui( state ){
		state = parseInt( state ) || 0; // 0 : 关闭， !0 ： 打开
	...
	function initDragItem(opt) {
		...
		_option.barDom.css({ cursor : 'move' });
		...
		_option.barDom.mousedown(function (event) {
			var isMove = true
				, abs_x = event.pageX - _option.targetDom.offset().left
				, abs_y = event.pageY - _option.targetDom.offset().top
			;

			$(document).bind('mousemove.shopDialog', function(event){
				var left = Math.max(0, event.pageX - abs_x)
					, top = Math.max(0, event.pageY - abs_y)
				;
				//不能超过 右、下边界
